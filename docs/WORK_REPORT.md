...
- [x] **LLM Decision**: Choice between ChatGPT API vs local LLM pending.
      I think the easest way to integrate AI is to use the ChatGPT API so le
- [x] **Deployment Strategy**: Local vs GitHub Actions not decided
      Lets use local aproach first which will generate the article and send it to backend.
- [ ] Create logic to fetch changes and all necesery data from github. It can be expanded to difrent git control services but at the moment lets focus on gitHub.
